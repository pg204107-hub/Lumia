...
export async function generateEmotionalLetter(data: MemoryData): Promise<LetterGenerationResponse> {
  ...
  Keep it under 250 words. Do not use generic phrases. Make it feel authentic, raw, and devastatingly beautiful. 
...
export async function generateMemoryPortrait(data: MemoryData, letterContent: string): Promise<string> {
  ...
  No text in image. Focus on symbolic elements mentioned in: ${letterContent.substring(0, 100)}`;
...
export async function generateLetterVoice(letter: string): Promise<string> {
  ...
  const response = await ai.models.generateContent({
    ...
    contents: [{ parts: [{ text: `Read this letter with a gentle, slow, and deeply emotional tone, pausing for breath: ${letter}` }] }],
    config: {
